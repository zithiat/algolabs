...
public class VariableLengthInt {
	...
	public static void operation(int a, int b, int result, String operation, int numberOfBits, int groupSize) {
		String astr = intToBinaryString(a, numberOfBits, groupSize);
		String line1 = String.format("%" + (operation.length() + 2 + astr.length()) + "s", astr);
		String line2 = String.format(" %s %s", operation, intToBinaryString(b, numberOfBits, groupSize));
		String line3 = String.format(" %s %s", "=", intToBinaryString(result, numberOfBits, groupSize));
	...
	public static void main(String[] args) {
		int[] arr = {35, 5, 8, 8, 1, 7, 6, 2};
		for (int it : arr) {
			byte[] a = encodeUInt32(it);
			for (byte aa : a) 
				printBinaryString(aa);
		}
	}
}
