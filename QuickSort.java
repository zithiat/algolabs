public class QuickSort {
	...
	/**
	 * Quick sort the given array starting from index {@code l} to {@code r}
	 * 
	...
	private static void sort(int[] array, int l, int r) {
		if (l < r) {
			...
			int j = r;
			while (i < j) {
				// System.out.println("i < j: " + i + " < " + j + "--r=" + r + "--l=" + l +
				// "--pivot=" + pivot + "--array[i]=" + array[i] + "--array[j]=" + array[j]);
				// Utils.printArray(array);
				// move right to avoid pivot element
				i += 1;
				// scan right: find elements greater than pivot
				while (i <= r && array[i] < pivot) {
					// System.out.println("scan right: i <= r && array[i] < pivot--i=" + i + "--r="
					// + r + "--j=" + j + "--l=" + l + "--pivot=" + pivot + "--array[i]=" + array[i]
					// + "--array[j]=" + array[j]);
					// Utils.printArray(array);
					i++;
				}
				...
				while (j >= l && array[j] > pivot) {
					// System.out.println("scan left: j >= l && array[j] > pivot--i=" + i + "--r=" +
					// r + "--j=" + j + "--l=" + l + "--pivot=" + pivot + "--array[i]=" + array[i] +
					// "--array[j]=" + array[j]);
					// Utils.printArray(array);
					j--;
				...
				if (i <= r && i < j) {
					// System.out.println("i <= r && i < j--i=" + i + "--r=" + r + "--j=" + j +
					// "--l=" + l + "--pivot=" + pivot + "--array[i]=" + array[i] + "--array[j]=" +
					// array[j]);
					// Utils.printArray(array);
					// swap around pivot
					swap(array, i, j);
